...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'confirm') {
    ...
    if (!$data) {
        setFlash('error', 'Registration session expired. Please start again.');
    ...
    $check = $db->prepare("SELECT id FROM institution_registrations WHERE email = ? AND status != 'rejected'");
    $check->execute([$data['email']]);
    if ($check->fetch()) {
        unset($_SESSION['reg_data']);
        setFlash('error', 'An institution registration already exists for this email address.');
        header('Location: ' . BASE_URL . '/app/register/index.php');
        exit;
    }
    ...
    // Create institution (pending_profile status)
    $db->prepare(
        "INSERT INTO institutions (registration_id, institution_name, address, status) VALUES (?,?,?,'pending_profile')"
    )->execute([$regId, $data['institution_name'], $data['address']]);
    $institutionId = (int)$db->lastInsertId();

    // Link registration to institution
    $db->prepare("UPDATE institution_registrations SET institution_id = ? WHERE id = ?")->execute([$institutionId, $regId]);
    ...
    // Send welcome email
    mailWelcome($data['email'], $data['spoc_name'], $data['institution_name'], $password);

    unset($_SESSION['reg_data']);

    $step     = 'done';
    $regEmail = $data['email'];
    $regName  = $data['institution_name'];
}
...
elseif ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'submit') {
    ...
    if (!$data['institution_name']) {
        ...
        // Check email not already used as institution admin
        $db   = getDB();
...
// ── Determine current step state for display ──────────────
$isForm   = ($step === 'form');
    ...
    <div class="row justify-content-center">
      <div class="col-lg-7 col-md-9">
        ...
        <div class="step-indicator mb-4">
          <div class="step <?= $isForm ? 'active' : 'done' ?>">
            <div class="step-circle">1</div>
            <div class="step-label">Registration</div>
          </div>
          <div class="step-line <?= !$isForm ? 'done' : '' ?>"></div>
            ...
            <form method="POST">
              ...
              <div class="mb-3">
                ...
                <input type="text" class="form-control" id="institution_name" name="institution_name"
                       ...
                       placeholder="e.g. Green Valley Sports Academy" required>
              </div>
        ...
        <div class="card">
          ...
          <div class="card-body p-4">
            <p class="text-muted small mb-4">
              Please review the information below carefully. Once confirmed, your institution will be created
              and login credentials will be sent to the registered email.
            </p>

            <div class="verify-card mb-4">
              <div class="verify-row"><span class="verify-key">Institution Name</span><span class="verify-val fw-600"><?= h($d['institution_name']) ?></span></div>
              <div class="verify-row"><span class="verify-key">SPOC / Admin Name</span><span class="verify-val"><?= h($d['spoc_name']) ?></span></div>
              <div class="verify-row"><span class="verify-key">Mobile Number</span><span class="verify-val">+91 <?= h($d['mobile']) ?></span></div>
              <div class="verify-row"><span class="verify-key">Admin Email</span><span class="verify-val"><?= h($d['email']) ?></span></div>
              <div class="verify-row"><span class="verify-key">Address</span><span class="verify-val"><?= nl2br(h($d['address'])) ?></span></div>
            </div>

            <div class="alert alert-info d-flex align-items-start gap-2 py-2 small">
              <i class="bi bi-info-circle-fill flex-shrink-0 mt-1"></i>
              <div>After confirmation, a login password will be emailed to <strong><?= h($d['email']) ?></strong>.
                   Log in and complete your institution profile to get approved by the Super Admin.</div>
            </div>
        ...
        <div class="card text-center">
          <div class="card-body p-5">
            ...
            <div class="alert alert-success d-inline-flex align-items-center gap-2 px-4 py-2 small mx-auto">
            ...
            <div class="mt-4">
              ...
              <div class="d-flex flex-column gap-2 text-start" style="max-width:380px;margin:0 auto;">
                <div class="d-flex align-items-center gap-2">
                  ...
                  <span class="small">Check your email and log in with the provided credentials</span>
                </div>
                <div class="d-flex align-items-center gap-2">
